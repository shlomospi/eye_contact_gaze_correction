...
class gaze_redirection_system:
    ...
    def shifting_angles_estimator(self, R_le, R_re,shared_v,lock):
        ...
        size_window = (659,528)
        Rw_lt = [int(Rs[0])/2-int(size_window[0]/2),
    ...
    def flx_gaze(self, frame, gray, detections, shared_v, lock, pixel_cut=[3,4], size_I = [48,64]):
        ...
        for k,bx in enumerate(detections):
            ...
            alpha_w2c, p_e, R_w = self.shifting_angles_estimator(LE_center,RE_center,shared_v,lock) # TODO is P_e required?
            ...
            frame[(R_le_LT[0]+pixel_cut[0]):(R_le_LT[0]+size_le_ori[0]-pixel_cut[0]),
                  (R_le_LT[1]+pixel_cut[1]):(R_le_LT[1]+size_le_ori[1]-pixel_cut[1])] = LE_infer[pixel_cut[0]:(-1*pixel_cut[0]),
                                                                                                 pixel_cut[1]:-1*(pixel_cut[1])]*255
            frame[(R_re_LT[0]+pixel_cut[0]):(R_re_LT[0]+size_re_ori[0]-pixel_cut[0]),
                  (R_re_LT[1]+pixel_cut[1]):(R_re_LT[1]+size_re_ori[1]-pixel_cut[1])] = RE_infer[pixel_cut[0]:(-1*pixel_cut[0]),
                                                                                                 pixel_cut[1]:-1*(pixel_cut[1])]*255
